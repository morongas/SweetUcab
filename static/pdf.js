...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    const responsePedido = await fetch(`/api/Pedido_Online/${claveped}`, {
...
async function renderUser(pedido) {
    ...
    const responseEstatus = await fetch(
        `/api/Estatus_Pedido_Online/PO_Clave/${claveped}`,
        {
            method: "GET",
        }
    );
    ...
    pedidoItem.innerHTML = `
        ...
        <center>
        <p></p>
        <h3>Monto = ${pedido[0].po_monto} Bs</h3>
        <h3>Fecha = ${pedido[0].po_fecha}</h3>
        <p></p>
    ...
    const responseDetalles = await fetch(
        `/api/Detalle_Pedido_Online/${pedido[0].po_clave}`,
        {
            method: "GET",
        }
    );
